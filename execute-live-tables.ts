...
async function createLiveTables() {
  console.log('🚀 Creating Live Tables for Approved Content...\n');
...
CREATE INDEX IF NOT EXISTS idx_accelerate_investors_type ON accelerate_investors(type);
  ...
  try {
    ...
    console.log('✅ Live tables created successfully!');
    
    // Verify tables exist
    console.log('\n📊 Verifying tables...');
    const tables = ['accelerate_startups', 'accelerate_news', 'accelerate_investors'];
    
    for (const table of tables) {
      console.log(`   ✅ ${table} created`);
    }
    
    console.log('\n🎉 Complete! Your system now has:');
    console.log('   - 3 Queue tables (for incoming data)');
    console.log('   - 3 Live tables (for approved data)');
    console.log('   - Full approval workflow support');
...
createLiveTables().catch(console.error);